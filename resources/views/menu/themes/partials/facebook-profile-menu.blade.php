...
<body class="font-['Cairo'] {{ $pageBackgroundClass ?? 'bg-slate-100' }} text-slate-900">
<main class="mx-auto w-full max-w-3xl p-2 pb-8 sm:p-4">
    <article class="overflow-hidden rounded-3xl border {{ $shellBorderClass ?? 'border-slate-200' }} {{ $shellBackgroundClass ?? 'bg-white' }} shadow-sm">
        <header>
            ...
            <div class="relative -mt-14 px-4 pb-4 sm:px-6">
                <div class="flex items-end justify-between gap-3">
                    <div class="flex items-end gap-3">
                        <div class="h-28 w-28 overflow-hidden rounded-full border-4 border-white bg-white shadow-md">
                            <img
                        ...
                        <div class="pb-2">
                            <h1 class="text-xl font-extrabold text-white drop-shadow-sm sm:text-2xl">{{ $restaurant->name }}</h1>
                            <p class="mt-1 max-w-md text-xs text-white/90 sm:text-sm">{{ $restaurant->description ?: 'منيو المطعم بطريقة سهلة وسريعة.' }}</p>
                        </div>
                    </div>

                    <button class="mb-2 inline-flex h-10 w-10 items-center justify-center rounded-full border border-white/60 bg-white/20 text-white backdrop-blur">
        ...
        <section class="px-4 pb-3 pt-1 sm:px-6">
            <div class="rounded-2xl border {{ $searchBorderClass ?? 'border-slate-200' }} {{ $searchBackgroundClass ?? 'bg-slate-50' }} px-3 py-2 text-sm text-slate-500">
        ...
        <section class="px-4 pb-4 sm:px-6">
            <h2 class="mb-3 text-sm font-bold text-slate-600">الأقسام</h2>
                ...
                <div class="flex min-w-max gap-3 px-1">
                    @foreach($categories as $category)
                        <a href="#cat-{{ $category->id }}" class="group w-20 text-center">
                            <span class="mx-auto inline-flex h-16 w-16 items-center justify-center overflow-hidden rounded-full border-2 {{ $storyBorderClass ?? 'border-slate-200' }} bg-white p-0.5 shadow-sm transition group-hover:scale-105">
                                @php
                            ...
                            <span class="mt-1 block line-clamp-1 text-xs font-semibold text-slate-600">{{ $category->name_ar }}</span>
                        </a>
                    @endforeach
                </div>
            </div>
        ...
        <section class="space-y-4 border-t border-slate-200 px-4 pb-5 pt-4 sm:px-6">
            @forelse($categories as $category)
                @if($category->products->isNotEmpty())
                    <section id="cat-{{ $category->id }}" class="rounded-2xl border border-slate-200 bg-white p-3 shadow-sm sm:p-4">
                        <h3 class="mb-3 text-base font-extrabold">{{ $category->name_ar }}</h3>
                            ...
                            @foreach($category->products as $product)
                                <article class="overflow-hidden rounded-2xl border border-slate-200 bg-white">
                                    ...
                                    <div class="space-y-2 p-3">
                                        <h4 class="line-clamp-1 text-sm font-extrabold">{{ $product->name }}</h4>
                                        <p class="line-clamp-2 text-xs text-slate-500">{{ $product->description ?: 'وصف بسيط للمنتج.' }}</p>
